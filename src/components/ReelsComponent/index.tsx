...
const ReelsComponent: React.FC<ReelsPropType> = ({
  reels,
  reelMetaInfo,
  onMenuItemClicked,
  onLikeClicked,
  ...
  const videoElementRefs = useRef<Array<RefObject<HTMLVideoElement>>>(
    reels.map(() => createRef<HTMLVideoElement>())
  );

  /* State Variables */
  const [currentVideoElementRef, setCurrentVideoElementRef] = useState<
    RefObject<HTMLVideoElement>
  >(videoElementRefs.current[0]); /* Ref. of Current Playing Video Element */
  const [isPlayingVideo, setIsPlayingVideo] =
    useState<boolean>(true); /* Video is playing or not */
  const [isAudioOn, setIsAudioOn] =
    useState<boolean>(false); /* Audio is on or not */
  ...
  useEffect(() => {
    ...
    let otherVideoElementRefs: Array<RefObject<HTMLVideoElement>> =
      videoElementRefs.current.filter((_, index: number) => index !== 0);
  ...
  const handleSlideChange = (event: any): void => {
    /* Current Video Ref. */
    let currentVideoElementRef: RefObject<HTMLVideoElement> | undefined =
      videoElementRefs.current.find(
        (_, index: number) => index === event.realIndex
      );
    /* Other Video Refs. */
    let otherVideoElementRefs: Array<RefObject<HTMLVideoElement>> =
      videoElementRefs.current.filter(
        (_, index: number) => index !== event.realIndex
      );
  ...
  return (
    <Fragment>
      <Swiper
        ...
        style={{
          height: "100vh",
          backgroundColor: reelMetaInfo?.backGroundColor || "#000000",
        }}
        direction={"vertical"}
        mousewheel={true}
        // spaceBetween={10}
              ...
              >
                ...
                >
...

                  {
                    ...
                    isAudioOn ? (
                      <GiSpeaker
                        ...
                        onClick={(e: any) => handleAudio(e, false)}
                      />
                    ) : (
                      <GiSpeakerOff
                        size={27}
                        className={styles.speakerOffIcon}
                        onClick={(e: any) => handleAudio(e, true)}
                      />
                    )
                    /* Audio On/Off buttons (Only for Desktop Screens) */
                  }

                  {
                    ...
                    isPlayingVideo ? (
                      <BsPlayFill size={55} className={styles.bigPlayIcon} />
                    ) : (
                      <GiPauseButton
                        size={50}
                  ...
                  <div
                    className={
                      sizeMode === sizeObj.extraSmallScreen
                        ? styles.sideBarIconsForSmallScreen
                        : styles.sideBarIcons
                    }
                  >
                    {/* Three Dot Menu Icon */}
                    ...
                    {reel.reelInfo.likes && (
                      <div onClick={() => handleLikeClick(reel)}>
                        ...
                        <span className={styles.likeText}>
                          {useNumberFilter(reelData[index].likes.count) !== "0"
                            ? useNumberFilter(reelData[index].likes.count)
                            : "Like"}
                        </span>
                      </div>
                    )}
                    ...
                    {reel.reelInfo.dislikes && (
                      <div onClick={() => handleDislikeClick(reel)}>
                        <FaThumbsDown
                          size={25}
                          color={
                            reelData[index].dislikes.isTap
                              ? reelMetaInfo?.dislikeActiveColor || "#3da6ff"
                              : "white"
                          }
                        />
                        <span className={styles.dislikeText}>
                          {useNumberFilter(reelData[index].dislikes.count) !==
                          "0"
                            ? useNumberFilter(reelData[index].dislikes.count)
                            : "Dislike"}
                        </span>
                    ...
                    {reel.reelInfo.comments && (
                      <div onClick={() => handleCommentClick(reel)}>
                        ...
                        <span className={styles.commentText}>
                          {useNumberFilter(reel.reelInfo.comments.count) !== "0"
                            ? useNumberFilter(reel.reelInfo.comments.count)
                            : "Comment"}
                        </span>
                      </div>
                    )}
                    ...
                    {reel.reelInfo.shares && (
                      <div onClick={() => handleShareClick(reel)}>
                        ...
                        <span className={styles.shareText}>
                          {useNumberFilter(reel.reelInfo.shares.count) !== "0"
                            ? useNumberFilter(reel.reelInfo.shares.count)
                            : "Share"}
                        </span>
                      </div>
                    )}
                      ...
                      )}
                      {
                        <div className={styles.productCard}>
                          <div className={styles.productCardBody}>
                            <div className={styles.productCardView}>
                              <div>
                                <img
                              ...
                              <div className={styles.productDescription}>
                                ...
                                >
                                  LifeBoost Organic Coffee - Tiramisu
                                </p>
                                <p
                                  style={{
                                    color: "#606060",
